...
### 3.3 AI 对话 UI（Vercel AI SDK）🔴
- ⬜ **[M]** 安装 Vercel AI SDK (`ai` 包)
- ⬜ **[M]** 实现 /api/chat 路由（调用 AgentCore API）
- ⬜ **[M]** 实现对话页面（useChat hook 流式渲染）
- ⬜ **[S]** 实现对话历史展示
...
### 4.2 Chat Agent（用户对话入口）🔴
- ⬜ **[M]** 实现 ChatAgent
  - ⬜ 处理用户自然语言查询
  - ⬜ 调用 tools 获取股票数据
  - ⬜ 触发深度分析（调用 orchestrator）
- ⬜ **[S]** 配置 System Prompt（英文，输出中文）
...
### 4.5 Agent 工具集成（统一定义）🔴
- 🟦 **[M]** 封装数据获取工具（所有 Agent 共享）
  - 🟦 tools/timestream.ts - get_stock_price, get_financials, get_news
  - 🟦 tools/indicators.ts - calculate_indicators
- ⬜ **[S]** 实现 trigger_analysis 工具（ChatAgent 调用 orchestrator）
- ⬜ **[S]** 配置工具权限（IAM）
- ⬜ **[S]** 实现工具调用日志

### 4.6 Agent 部署 (CDK + AgentCore) 🔴
- ✅ **[M]** 创建 Express 服务器入口 (index.ts)
  - ✅ /ping 健康检查端点
  - ✅ /invocations 调用端点
- ✅ **[M]** 创建 Dockerfile
- ⬜ **[M]** 在 backend.ts 添加 AgentCore CDK
- ⬜ **[S]** 配置 AgentCore Memory（STM + LTM）
- ⬜ **[S]** 测试 Agent 协作流程
